    ...
    def _get_mask(self, current_steps: int, device: torch.device, dtype: torch.dtype):
        ...
        if self.memory_efficient:
            if current_steps == 1:
                ...
                # Return None to signal causal attention (is_causal=True in SDPA)
                return True
        ...
        if self.custom:
            ...
            if self.memory_efficient:
                ...
                if custom_attn_mask and isinstance(attn_mask, torch.Tensor):
                    ...
                    sdpa_mask = attn_mask.to(q.dtype)
                    sdpa_mask = sdpa_mask.repeat((q.shape[0], 1, 1, 1))
                    sdpa_mask = sdpa_mask[..., :seq_len, :seq_len]
                elif attn_mask is not None:
                    is_causal = True

                p = self.dropout if self.training else 0
                if _efficient_attention_backend == 'torch' or ops is None:
                    x = torch.nn.functional.scaled_dot_product_attention(
                        q, k, v, attn_mask=sdpa_mask, is_causal=is_causal, dropout_p=p)
                else:
