...
class MusicGen(BaseGenModel):
    ...
    def get_pretrained(name: str = 'facebook/musicgen-melody', device=None):
        ...
        # On MPS, limit max_duration to 10s so the sliding window kicks in
        # for longer generations (avoids OOM from MPS attention memory spikes).
        mps_device = (device == 'mps') if device is not None else torch.backends.mps.is_available()
        max_dur = 10.0 if mps_device else None
        return MusicGen(name, compression_model, lm, max_duration=max_dur)
    ...
    def set_generation_params(self, use_sampling: bool = True, top_k: int = 250,
                              ...
                              extend_stride: tp.Optional[float] = None):
        ...
        Args:
            ...
            extend_stride: when doing extended generation (i.e. more than max_duration), by how much
                should we extend the audio each time. Larger values will mean less context is
                preserved, and shorter value will require extra computations.
        """
        if extend_stride is None:
            extend_stride = self.max_duration - 6  # 6s overlap per chunk for musical continuity
