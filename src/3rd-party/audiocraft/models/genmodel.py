...
class BaseGenModel(ABC):
    ...
    def generate_audio(self, gen_tokens: torch.Tensor) -> torch.Tensor:
        ...
        On MPS devices, the decoder (EnCodec) is run on CPU to avoid
        unsupported FFT ops and memory leaks from MPS fallback copies.
        """
        assert gen_tokens.dim() == 3
        with torch.no_grad():
            if gen_tokens.device.type == 'mps':
                # Move tokens + decoder to CPU for decoding (FFT not supported on MPS)
                self.compression_model.to('cpu')
                gen_audio = self.compression_model.decode(gen_tokens.to('cpu'), None)
                self.compression_model.to('mps')
            else:
